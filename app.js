...
$('#cloud-login').onclick=async()=>{if(window.MINDFOLD_DIRECT_B2){try{MindfoldB2.askCredentials();await MindfoldB2.testConnection();showToast('B2 已连接');loadAttachments();}catch(error){if(error.message!=='B2 credentials required')showToast(`B2 连接失败：${error.message}`);}return;}if(!SYNC_ORIGIN){showToast('请先配置云端同步服务');return;}window.location.href=`${SYNC_ORIGIN}/auth/login`;};
