...
function render(){
  ...
  layer.style.transform=`scale(${zoom})`; drawConnections(); updateInspector(); $('#node-count').textContent=`${map.nodes.length} 个节点`; save();
}
...
function startDrag(e){ if(document.querySelector('[data-tool].active')?.dataset.tool==='hand') return; e.stopPropagation(); selectNode(e.currentTarget.dataset.id); const node=nodeById(selectedId); dragging={node, startX:e.clientX, startY:e.clientY, x:node.x, y:node.y}; try{ e.currentTarget.setPointerCapture(e.pointerId); }catch{} e.currentTarget.addEventListener('pointermove', drag); e.currentTarget.addEventListener('pointerup', stopDrag, {once:true}); }
...
function stopDrag(e){ dragging=null; e.currentTarget.removeEventListener('pointermove',drag); save(); }
...
canvas.addEventListener('dblclick',e=>{if(e.target!==canvas)return; const rect=canvas.getBoundingClientRect(); map.nodes.push({id:`node-${Date.now()}`,text:'新的想法',x:(e.clientX-rect.left)/zoom-65,y:(e.clientY-rect.top)/zoom-24,color:'yellow',parent:null}); selectedId=map.nodes.at(-1).id;render();}); canvas.addEventListener('pointerdown',e=>{if(e.target!==canvas||document.querySelector('[data-tool].active')?.dataset.tool!=='hand')return;panning={x:e.clientX,y:e.clientY,scrollX:canvas.scrollLeft,scrollY:canvas.scrollTop};}); canvas.addEventListener('pointermove',e=>{if(panning){canvas.scrollLeft=panning.scrollX-(e.clientX-panning.x);canvas.scrollTop=panning.scrollY-(e.clientY-panning.y)}}); canvas.addEventListener('pointerup',()=>panning=null);
