...
async function syncRemote(){ if(syncing)return; syncing=true; try{ if(window.MINDFOLD_DIRECT_B2){await MindfoldB2.statePut({maps,activeMapId});stateLabel.textContent='已同步';}else{ const response=await fetch(SYNC_URL,apiOptions({method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({maps,activeMapId})})); if(response.ok) stateLabel.textContent='已同步'; }}catch(error){if(error.message!=='B2 credentials required')showToast(`同步失败：${error.message}`);} finally{syncing=false;} }
async function pullRemote(){ try{const remote=window.MINDFOLD_DIRECT_B2?await MindfoldB2.stateGet():await (async()=>{const response=await fetch(SYNC_URL,apiOptions({cache:'no-store'}));return response.ok?response.json():null;})(); if(remote?.maps&&JSON.stringify(remote.maps)!==JSON.stringify(maps)){maps=remote.maps;activeMapId=remote.activeMapId||Object.keys(maps)[0];map=maps[activeMapId];titleInput.value=map.title;render();showToast('已同步最新数据');}}catch(error){if(error.message!=='B2 credentials required')showToast(`读取失败：${error.message}`);} }
async function loadAttachments(){ try{if(window.MINDFOLD_DIRECT_B2){attachments=await MindfoldB2.files();renderAttachments();return;}const response=await fetch(`${SYNC_ORIGIN}/api/files`,apiOptions({cache:'no-store'})); if(response.ok){attachments=await response.json();renderAttachments();}}catch(error){if(error.message!=='B2 credentials required')showToast(`读取附件失败：${error.message}`);} }
...
$('#cloud-login').onclick=async()=>{if(window.MINDFOLD_DIRECT_B2){try{MindfoldB2.clear();MindfoldB2.askCredentials();await MindfoldB2.testConnection();showToast('B2 已连接');loadAttachments();}catch(error){if(error.message!=='B2 credentials required')showToast(`B2 连接失败：${error.message}`);}return;}if(!SYNC_ORIGIN){showToast('请先配置云端同步服务');return;}window.location.href=`${SYNC_ORIGIN}/auth/login`;};
